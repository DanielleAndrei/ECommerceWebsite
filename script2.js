...
function reloadCart() {
    ...
    listCarts.forEach((value, key) => {
        if (value != null) {
            ...
            newDiv.innerHTML = `
                <div class="cart-item">
                    <img src="${value.image}" class="cart-item-image"/>
                    <div class="cart-item-details">
                        <div class="cart-item-name">${value.name}</div>
                        <div class="cart-item-price">₱${value.price.toLocaleString()}</div>
...
function redirectToCheckout() {
    if (quantity.innerText != '0') {
        localStorage.setItem('checkoutItems', JSON.stringify(listCarts));
        window.location.href = 'index1.html';
    }
    else
    {
        displayNotification("No Items in the Cart")
    }
}
